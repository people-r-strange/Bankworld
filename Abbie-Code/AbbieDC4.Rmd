...
```{r}
odd_ips_46 <- firewall46 %>% 
  filter(!grepl("172.", dest_ip) & !grepl("10.", dest_ip) & dest_ip != "(empty)")

View(odd_ips_46)
         
odd_ips_47 <- firewall47 %>% 
  filter(!grepl("172.", dest_ip) & !grepl("10.", dest_ip) & dest_ip != "(empty)")

View(odd_ips_47)

fin_firewall47 <- firewall47 %>% 
  filter(grepl("172.23.220", source_ip) | grepl("172.23.220", dest_ip))
...
#   count(timestamp)
# 
# critical_plot <- ggplot(critical_count, aes(x = timestamp, y = n)) +
#   geom_point()
# 
# critical_plot
```
...
#   group_by(source_ip, byMin = cut(timestamp, "1 min")) %>% 
#   summarize(count = n())
# 
# plot <- ggplot(count, aes(x = byMin, y = count)) +
#   geom_point() +
#   facet_wrap(~source_ip)
# plot

# sus_ip_count <- all_sus_ips %>% 
#   group_by(source_ip, timestamp, syslog_priority) %>%
#   filter(syslog_priority != "Info") %>% 
#   count(timestamp)
# 
# sus_ip_plot <- ggplot(sus_ip_count, aes(x = timestamp, y = n, color = source_ip)) +
#   geom_line()
# sus_ip_plot
