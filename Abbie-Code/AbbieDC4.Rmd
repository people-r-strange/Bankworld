...
#   group_by(source_ip, timestamp) %>%
#   count(timestamp)
# 
# sus_ip_plot <- ggplot(sus_ip_count, aes(x = timestamp, y = n)) +
#   geom_line() +
#   geom_smooth()
# sus_ip_plot
...
bad_port_count <- bad_ports %>% 
  group_by(dest_port, dest_service) %>% 
  count(dest_port)

bad_port_plot <- ggplot(bad_port_count, aes(x = dest_service, y = n)) + 
  geom_col()
bad_port_plot
```
